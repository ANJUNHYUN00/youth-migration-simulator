...
> **목적**: 미션 이동 화면(MissionTravelingScreen)에서 사용자가 일러스트와 비교해서 볼 수 있는 **실제 네이버 로드뷰 사진**을 수집해야 합니다.
...
- **네이버 로드뷰**: 캡처 → 워터마크/UI 자르기 → 가공
- **저작권**: 네이버 로드뷰 출처 표기 필요 시 모달에 caption 추가 (개발팀과 상의)
